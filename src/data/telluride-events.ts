/**
 * Telluride Events Data
 * Extracted from official Telluride events page
 * Used by EventsWidget and TripCalculator
 */

export interface TellurideEvent {
  id: string;
  name: string;
  date: string;
...
// Past events are automatically filtered out based on current date in components
export const TELLURIDE_EVENTS: TellurideEvent[] = [
  {
    id: '1',
    name: 'Turkey Trot',
    date: '2025-11-27',
 ...
 */
export function getEventsInRange(startDate: string, endDate: string): TellurideEvent[] {
  const start = new Date(startDate);
  const end = new Date(endDate);
  
  return TELLURIDE_EVENTS.filter(event => {
 ...
 */
export function getUpcomingEvents(limit?: number): TellurideEvent[] {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  const upcoming = TELLURIDE_EVENTS
