  ...
  return (
    <div className="bg-gradient-to-br from-sky-50 via-white to-sky-50 rounded-2xl p-6 border border-sky-200 shadow-card h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-xl font-black text-neutral-900">Live Webcams</h3>
          <p className="text-sm font-semibold text-neutral-600">See current conditions</p>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                {/* Icon */}
                <div className="flex-shrink-0 mt-1 w-10 h-10 rounded-lg bg-sky-100 flex items-center justify-center shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <h4 className="font-black text-neutral-900 group-hover:text-sky-600 transition-colors">
                      {source.name}
                    </h4>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {source.locations.map((location) => (
                      <span
                        key={location}
                        className="px-2.5 py-1 bg-sky-50 text-sky-700 text-xs font-bold rounded-md"
                      >
                        {location}
                      </span>
                    ))}
      ...
      <div className="mt-4 p-3 bg-sky-50 border border-sky-200 rounded-lg flex items-start gap-2">
        ...
        <p className="text-xs text-sky-900 font-semibold">
          <span className="font-black">Tip:</span> Webcams are updated every few minutes. Check conditions before heading to the mountain!
        </p>
      </div>
    </div>
  );
