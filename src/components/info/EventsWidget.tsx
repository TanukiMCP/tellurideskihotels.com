...
export function EventsWidget() {
  ...
  return (
    <article className="bg-white rounded-2xl p-8 border border-[#E8E8E8] shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-200 flex flex-col h-full">
      ...
      {/* Events List - Shows next 2-3 upcoming events */}
            ...
            return (
              ...
              >
                ...
                <div className="flex-shrink-0 w-16 h-16 rounded-xl bg-[#E8F2ED] border border-[#2D5F4F] flex flex-col items-center justify-center">
                  <div className="text-xs font-semibold text-[#2D5F4F] tracking-wide uppercase">
                    {month}
                  </div>
                  <div className="text-2xl font-bold text-[#2D5F4F] leading-none mt-1">
                    {day}
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <h4 className="text-base font-semibold text-[#2C2C2C] group-hover:text-[#2D5F4F] transition-colors mb-1">
                      {event.name}
                    </h4>
                    <svg className="w-4 h-4 text-[#8BA888] group-hover:text-[#2D5F4F] transition-colors flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                    </svg>
                  </div>
                  <p className="text-sm text-[#666] leading-relaxed line-clamp-2">{event.description}</p>
                </div>
              </a>
            );
          })
