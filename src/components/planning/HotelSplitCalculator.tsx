...
export function HotelSplitCalculator({
  ...
  const fetchRatesAndCalculate = async () => {
    try {
      ...
      if (hotels.length === 0) {
        calculateComparisons(avgHotelRate); // Use existing rate
        return;
      }
      ...
      if (!ratesResponse.ok) {
        calculateComparisons(avgHotelRate); // Use existing rate if rates API fails
        return;
      }
      ...
      const calculatedRate = rateCount > 0 ? totalRate / rateCount : avgHotelRate;
      setAvgHotelRate(calculatedRate);
      calculateComparisons(calculatedRate);
      setError(null);
    } catch (err) {
