...
export function TripCalculator({
  ...
  // Get events in date range using shared data
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'My Telluride Trip Plan',
      text: `${guests} travelers • ${nights} nights (${format(parseISO(checkIn), 'MMM d')} - ${format(parseISO(checkOut), 'MMM d')}) • ${formatCurrency(budget)} budget`,
  ...
  return (
    <div ref={containerRef} className="bg-white rounded-2xl shadow-xl border border-neutral-200 overflow-hidden print:shadow-none print:border-0">
      {/* Header */}
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 text-white p-6 print:bg-primary-600">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-white/10 backdrop-blur rounded-xl flex items-center justify-center print:bg-white/20">
            <Calculator className="w-7 h-7 text-white" />
          </div>
          <div className="flex-1">
            <h2 className="text-2xl font-bold">{title}</h2>
            <p className="text-primary-100 text-sm">Set your budget and preferences to find what fits</p>
          </div>
          <img 
            src="/favicon-icon.png" 
            alt="Telluride Insider" 
            className="h-10 w-auto opacity-90 hidden sm:block print:block"
          />
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-2 flex-wrap print:hidden">
          ...
          <div>
            ...
            <div className="flex items-center justify-between bg-white border-2 border-neutral-200 rounded-xl px-4 py-3">
              <button 
                ...
                className="w-10 h-10 rounded-lg bg-neutral-100 hover:bg-neutral-200 flex items-center justify-center transition-colors print:hidden"
              >
        ...
        {/* Category Toggles */}
        <div className="bg-neutral-50 rounded-xl p-4 print:bg-neutral-100">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                ...
                disabled={locked}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all print:border ${
                  categories[key as keyof TripCategories]
                    ? 'bg-primary-600 text-white print:bg-primary-100 print:text-primary-700 print:border-primary-300'
        ...
        {categories.skiing && (
          <div className="bg-sky-50 rounded-xl p-4 border border-sky-200">
            ...
            {(showSkiDetails || typeof window === 'undefined') && (
              <div className="mt-4 space-y-4 print:mt-0">
                {!tripInSkiSeason ? (
                  ...
                  <>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => setAdultSkiers(Math.max(0, adultSkiers - 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Minus className="w-4 h-4" /></button>
                          <span className="text-lg font-bold w-8 text-center">{adultSkiers}</span>
                          <button onClick={() => setAdultSkiers(Math.min(guests, adultSkiers + 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Plus className="w-4 h-4" /></button>
                        </div>
                      </div>
                      {/* Child Skiers */}
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => setChildSkiers(Math.max(0, childSkiers - 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Minus className="w-4 h-4" /></button>
                          <span className="text-lg font-bold w-8 text-center">{childSkiers}</span>
                          <button onClick={() => setChildSkiers(Math.min(guests - adultSkiers, childSkiers + 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Plus className="w-4 h-4" /></button>
                        </div>
                      </div>
                      {/* Toddlers */}
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => setToddlerSkiers(Math.max(0, toddlerSkiers - 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Minus className="w-4 h-4" /></button>
                          <span className="text-lg font-bold w-8 text-center">{toddlerSkiers}</span>
                          <button onClick={() => setToddlerSkiers(Math.min(guests - adultSkiers - childSkiers, toddlerSkiers + 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Plus className="w-4 h-4" /></button>
                        </div>
                      </div>
                      {/* Ski Days */}
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => setSkiDays(Math.max(1, skiDays - 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Minus className="w-4 h-4" /></button>
                          <span className="text-lg font-bold w-8 text-center">{skiDays}</span>
                          <button onClick={() => setSkiDays(Math.min(nights, skiDays + 1))} className="w-8 h-8 rounded bg-white border border-neutral-200 flex items-center justify-center hover:bg-neutral-50 print:hidden"><Plus className="w-4 h-4" /></button>
                        </div>
                      </div>
                    </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-neutral-200">
          <button 
            ...
            className="w-full flex items-center justify-between text-left mb-3 print:hidden"
          >
          ...
          <div className="h-6 rounded-full overflow-hidden flex mb-4">
            ...
            {categories.lodging && (
              <div className="bg-primary-500 flex items-center justify-center text-white text-xs font-medium" style={{ width: `${(budgetBreakdown.lodging.total / budget) * 100}%` }}>Lodging</div>
            )}
            {categories.activities && (
              <div className="bg-secondary-500 flex items-center justify-center text-white text-xs font-medium" style={{ width: `${(budgetBreakdown.activities.total / budget) * 100}%` }}>Activities</div>
            )}
            {categories.dining && (
              <div className="bg-accent-500 flex items-center justify-center text-white text-xs font-medium" style={{ width: `${(budgetBreakdown.dining.total / budget) * 100}%` }}>Dining</div>
            )}
          </div>

          {/* Allocation Controls */}
          {(showAllocation || typeof window === 'undefined') && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-neutral-100 print:grid">
              {categories.lodging && (
        ...
        {/* Results: Hotels */}
        {categories.lodging && (
          <div className="border-t border-neutral-200 pt-6 print:break-before-page">
            <HotelGrid 
            ...
            <div className="mt-4 text-center print:hidden">
              <a href={`/places-to-stay?maxPrice=${budgetBreakdown.lodging.perNight}&checkin=${checkIn}&checkout=${checkOut}&guests=${guests}`} className="inline-flex items-center gap-2 text-primary-600 hover:text-primary-700 font-semibold">
                Browse All Hotels <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
              </a>
            </div>
          </div>
        )}
        ...
        {categories.activities && (
          <div className="border-t border-neutral-200 pt-6">
            ...
            />
            <div className="mt-4 text-center print:hidden">
              <a href="/things-to-do" className="inline-flex items-center gap-2 text-primary-600 hover:text-primary-700 font-semibold">
                Explore All Activities <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
              </a>
            </div>
          </div>
        )}
          ...
          <div className="border-t border-neutral-200 pt-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-500">{format(parseISO(checkIn), 'MMM d')} - {format(parseISO(checkOut), 'MMM d, yyyy')}</p>
              </div>
            </div>
            
            {tripEvents.length > 0 ? (
              <div className="space-y-3">
                {tripEvents.slice(0, 4).map(event => (
                  <a key={event.id} href={event.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-4 bg-neutral-50 rounded-lg hover:bg-purple-50 transition-colors group print:bg-neutral-100">
                    <div className="w-14 h-14 bg-purple-100 rounded-lg flex flex-col items-center justify-center flex-shrink-0">
                      <span className="text-xs font-semibold text-purple-700">{format(parseISO(event.date), 'MMM')}</span>
                      <span className="text-xl font-bold text-purple-700">{format(parseISO(event.date), 'd')}</span>
                    </div>
                    <div className="flex-1">
                      <h4 className="font-semibold text-neutral-900 group-hover:text-purple-700">{event.name}</h4>
                      <p className="text-sm text-neutral-600">{event.description}</p>
                    </div>
                  </a>
                ))}
                {tripEvents.length > 4 && (
                  <p className="text-sm text-neutral-500 text-center">+{tripEvents.length - 4} more events during your trip</p>
                )}
              </div>
            ) : (
            ...
            <div className="mt-4 text-center print:hidden">
              <a href="https://www.telluride.com/festivals-events/events/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-purple-600 hover:text-purple-700 font-semibold">
                View Full Event Calendar <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
              </a>
            </div>
          </div>
        )}
      ...
      {/* Trip Summary / Export Section */}
      <div ref={summaryRef} className="bg-gradient-to-r from-primary-600 to-primary-700 p-6 text-white">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <img src="/favicon-icon.png" alt="Telluride Insider" className="h-8 w-auto mb-2 opacity-90" />
            <h3 className="text-xl font-bold">Your Telluride Trip</h3>
          </div>
          <div className="text-right text-sm text-primary-100">
            <p>tellurideinsider.com</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 text-center">
          <div className="bg-white/10 rounded-lg p-3">
            ...
            <p className="text-xs text-primary-200">Travelers</p>
          </div>
          <div className="bg-white/10 rounded-lg p-3">
            <p className="text-2xl font-bold">{nights}</p>
            <p className="text-xs text-primary-200">Nights</p>
          </div>
          <div className="bg-white/10 rounded-lg p-3">
            <p className="text-lg font-bold">{format(parseISO(checkIn), 'MMM d')}</p>
            <p className="text-xs text-primary-200">Check-in</p>
          </div>
          <div className="bg-white/10 rounded-lg p-3">
            <p className="text-lg font-bold">{format(parseISO(checkOut), 'MMM d')}</p>
            <p className="text-xs text-primary-200">Check-out</p>
          </div>
        </div>
        ...
        <div className="bg-white/10 rounded-lg p-4 mb-4">
          ...
          {budgetBreakdown.fixedCosts > 0 && (
            <div className="flex justify-between items-center text-sm text-primary-200">
              <span>Lift Tickets ({adultSkiers + childSkiers} skiers × {skiDays} days)</span>
              <span>{formatCurrency(budgetBreakdown.fixedCosts)}</span>
            </div>
          )}
          {categories.lodging && (
            <div className="flex justify-between items-center text-sm text-primary-200">
              <span>Lodging ({formatCurrency(budgetBreakdown.lodging.perNight)}/night)</span>
              <span>{formatCurrency(budgetBreakdown.lodging.total)}</span>
            </div>
          )}
          {categories.activities && (
            <div className="flex justify-between items-center text-sm text-primary-200">
              <span>Activities ({formatCurrency(budgetBreakdown.activities.perPerson)}/person)</span>
              <span>{formatCurrency(budgetBreakdown.activities.total)}</span>
            </div>
          )}
          {categories.dining && (
            <div className="flex justify-between items-center text-sm text-primary-200">
              <span>Dining ({formatCurrency(budgetBreakdown.dining.perPersonPerDay)}/person/day)</span>
              <span>{formatCurrency(budgetBreakdown.dining.total)}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 print:hidden">
          <button
            ...
            className="inline-flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white font-semibold px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
          >
          ...
          <button
            onClick={handleShare}
            className="inline-flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white font-semibold px-4 py-2 rounded-lg transition-colors"
          >
