...
// Seasonal satellite styles - only used when satellite view is selected
const SEASONAL_SATELLITE_STYLES = {
  winter: 'mapbox://styles/mapbox/satellite-streets-v12', // Satellite view for winter conditions
  summer: 'mapbox://styles/mapbox/satellite-streets-v12', // Satellite view for summer conditions
  // Note: Both use same style for now, but structure allows for future differentiation
};
...
export default function InteractiveTrailMap() {
  ...
  const [season, setSeason] = useState<'winter' | 'summer'>('winter'); // Only used when satellite is selected
          ...
          if (terrainEnabled) {
            ...
            if (currentPitch < 45) {
              map.easeTo({
                ...
                bearing: map.getBearing() || SUMMIT_3D_VIEWPOINT.bearing,
                duration: 500
              });
            }
          }
  ...
  useEffect(() => {
    ...
    const addTerrainAndLayers = () => {
      // Add terrain sources for BOTH styles - satellite can have 3D terrain too!
      // This allows 3D satellite view with custom layers overlaid
      if (!map.getSource('local-terrain')) {
        try {
          ...
          console.log('[InteractiveTrailMap] Local terrain source already exists or error:', err);
        }
      }
      ...
      if (!map.getSource('mapbox-dem')) {
        try {
          ...
          console.log('[InteractiveTrailMap] Mapbox DEM source already exists or error:', err);
        }
      }
      
      // Re-apply terrain if it was previously enabled (works for both styles)
      if (terrainEnabled) {
        // Small delay to ensure source is ready
        setTimeout(() => {
          if (map.getSource('mapbox-dem')) {
            // Apply terrain to current style (outdoors or satellite)
            map.setTerrain({ source: 'mapbox-dem', exaggeration: 2.0 });
            ...
            console.log('[InteractiveTrailMap] Re-applied 3D terrain to', mapStyle, 'style');
          }
        }, 200);
      }
  ...
  }, [isMapLoaded, mapStyle, season, showTrails, showLifts, showPOIs, terrainEnabled]);
  ...
  const toggle3D = () => {
    ...
    if (newTerrainState) {
      ...
      if (!demSource) {
        console.warn('[InteractiveTrailMap] mapbox-dem source not found, adding it...');
        ...
        try {
          ...
          console.error('[InteractiveTrailMap] Failed to add mapbox-dem source:', err);
          return;
        }
      }
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full ${isFullscreen ? 'bg-neutral-900' : ''}`}>
      ...
      <Map
        ...
        mapStyle={mapStyle === 'satellite' ? SEASONAL_SATELLITE_STYLES[season] : MAP_STYLES[mapStyle]}
      ...
      >
        ...
        {showLeftPanel ? (
          <div className="absolute top-4 left-4 z-10 bg-white rounded-xl shadow-2xl border-2 border-neutral-800 max-w-sm flex flex-col max-h-[calc(100vh-2rem)]" style={{fontFamily: 'system-ui, -apple-system, sans-serif'}}>
            ...
            {/* Season Toggle - Only shown when Satellite is selected */}
            {mapStyle === 'satellite' && (
              <div className="mb-4 pb-4 border-b border-neutral-300">
                ...
                <p className="text-[10px] text-neutral-500 mt-2 text-center">
                  {season === 'winter' ? 'Winter satellite view' : 'Summer satellite view'}
                </p>
              </div>
            )}
